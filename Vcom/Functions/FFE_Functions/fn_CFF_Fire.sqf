...
if (_aGuns < 1 || _amount < 1) exitWith {-1};
	
private _perGun = floor (_amount/_aGuns);
private _rest = _amount - (_perGun * _aGuns);

{
	private _shots = _x getVariable ["RydFFE_MyShots",0];
...
while {_rest > 0} do
{
	...
	
	{
		...
		if (_shots > 0) then
		{
			private _toFire = _x getVariable ["RydFFE_ShotsToFire",0];
	...
	if (_rest0 <= _rest && _rest > 0) exitWith {_bad = true}
};
		...
		
{
	if (!isNull _x) then
	{
		...
		if ((_vh getVariable ["RydFFE_ShotsToFire",0]) > 0) then
		{
			...
			private _ammoC = (magazines _vh) select 0;
					...
					if (_pos inRangeOfArtillery [[_vh],_ammo]) then
					{
						if (_ammo in (getArtilleryAmmo [_vh])) then
						{
							...
							if !((currentMagazine _vh) in [_ammo]) then
							{
								...
								{
									sleep 0.1;
									private _ws = weaponState [_vh,[0]];
									_ws = _ws select 3;
									(_ws in [_ammo] || (time - _ct) > 30)
								};
							...
							if (_pos inRangeOfArtillery [[_vh],_ammo]) then
							{
								if (_ammo in (getArtilleryAmmo [_vh])) then
								{																				
									if (((toLower (typeOf _vh)) in ["uss_iowa_turret_c","uss_iowa_turret_b","uss_iowa_turret_a"]) || {RydFFE_IowaMode}) then
									{
										{
									...
									{
										sleep 0.1;
										(!((_vh getVariable ["RydFFE_ShotFired2",0]) < (_vh getVariable ["RydFFE_ShotsToFire",1])) || ((time - _ct) > 15))
									};
